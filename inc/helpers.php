 ...
 * ổn định giữa các trang (tránh sản phẩm nhảy/lặp khi phân trang). SP để trống
 * coi như 0 — giữ nhóm chưa ghim ở cuối, sắp theo bài mới nhất.
 *
 * Ngữ nghĩa này khớp UI "Thứ tự hiển thị" trong wp-admin (inc/admin-product-order):
 * admin gõ số càng lớn cho SP muốn đẩy lên đầu; SP còn lại bỏ trống.
 *
...
function okhub_product_order_clauses()
{
    ...
    return function ($clauses, $wp_query) use ($wpdb) {
        ...
        // index lớn lên trước; trống/không có coi như 0 (xuống cuối). Chốt bằng
        // post_date/ID cho ổn định phân trang.

...
class Header {
  ...
  constructor() {
    ...
    // Chỉ một số trang (home/about) có header transparent lúc đầu,
    // sau đó đổi sang header--white khi scroll.
    this.isTransparentHeaderPage =
      document.body.classList.contains("home") ||
      document.body.classList.contains("page-template-about-us-page") ||
      document.body.classList.contains("page-template-faqs") ||
      document.body.classList.contains("page-template-faqs-php") ||
      document.body.classList.contains("single-service");

    if (!this.isTransparentHeaderPage) {
    ...
    window.addEventListener("scroll", this.scheduleScrollCheck, {
      passive: true,
    });
    window.addEventListener("resize", this.scheduleScrollCheck, {
      passive: true,
    });
...
class HeaderMegaMenuService {
  constructor() {
    this.megaMenuServiceEls = document.querySelectorAll(
      ".header__mega-menu-service"
    );
    if (!this.megaMenuServiceEls.length) return;
    this.events();
  }
  setActiveByIndex(container, index) {
    if (!container) return;
    const idx = Number(index);
    if (!Number.isFinite(idx)) return;

    const activeServiceItem = container.querySelector(
      ".header__mega-menu-service__service-item--active"
    );
    if (activeServiceItem) {
      activeServiceItem.classList.remove(
        "header__mega-menu-service__service-item--active"
      );
    }
    const nextItem = container.querySelector(
      `[data-service-trigger-index="${idx}"]`
    );
    if (nextItem) {
  ...
  handleHoverServiceItem() {
    this.megaMenuServiceEls.forEach((container) => {
      const serviceItemEls = container.querySelectorAll(
        ".header__mega-menu-service__service-item"
      );
      serviceItemEls.forEach((serviceItem) => {
        serviceItem.addEventListener("mouseenter", () => {
          const idx = serviceItem.getAttribute("data-service-trigger-index");
          this.setActiveByIndex(container, idx);
        });
      });
    });
  }
...
class HeaderMegaMenuProduct {
  ...
  constructor() {
    this.container = document.querySelector(
      '[data-mega-menu-content="mega-menu-product"]'
    );
    if (!this.container) return;

    this.parentSwiperEl = this.container.querySelector(
      ".header__mega-menu-product__parent-categories-swiper"
    );
    this.childSwiperEl = this.container.querySelector(
      ".header__mega-menu-product__child-categories-swiper"
    );
    this.prevBtn = this.container.querySelector(
      ".header__mega-menu-product__parent-categories-swiper-prev"
    );
    this.nextBtn = this.container.querySelector(
      ".header__mega-menu-product__parent-categories-swiper-next"
    );
    this.scrollbarTrack = this.container.querySelector(
      ".header__mega-menu-product__child-categories-swiper-scrollbar"
    );
    this.scrollbarThumb = this.container.querySelector(
      ".header__mega-menu-product__child-categories-swiper-scrollbar-inner"
    );
  ...
  bindCustomScrollbar() {
    ...
    const updateScrollbar = () => {
      ...
      if (scrollHeight <= clientHeight) {
        ...
        thumb.style.top = "0";
        track.classList.add(
          "header__mega-menu-product__child-categories-swiper-scrollbar--no-scroll"
        );
        return;
      }
      ...
      const thumbTop =
        scrollableRange > 0 ? (scrollTop / scrollableRange) * thumbTravel : 0;
    ...
    track.addEventListener("mousedown", (e) => {
      ...
      if (clickY < thumbTop) {
        ...
        scrollEl.scrollTop = Math.min(
          scrollableRange,
          scrollEl.scrollTop + clientHeight
        );
      }
    });
  }
...
class HeaderMegaMenuSearch {
  ...
  static TRANSPARENT_PIXEL_DATA_URL =
    "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==";

  constructor() {
    this.containerEl = document.querySelector(
      '[data-mega-menu-content="mega-menu-search"]'
    );
    ...
    this.inputEl = this.containerEl.querySelector(
      ".header__mega-menu-navbar__searchbar-input"
    );
    this.resultContainerEl = this.containerEl.querySelector(
      ".header__mega-menu-search__result"
    );
    this.resultListEl = this.containerEl.querySelector(
      ".header__mega-menu-search__result-list"
    );
    this.relatedBlogEl = this.containerEl.querySelector(
      ".header__mega-menu-search__related-blog"
    );
    this.relatedBlogListEl = this.containerEl.querySelector(
      ".header__mega-menu-search__related-blog-list"
    );
    this.noResultEl = this.containerEl.querySelector(
      ".header__mega-menu-search__no-result"
    );
    this.productNoResultEl = this.containerEl.querySelector(
      ".header__mega-menu-search__product-no-result"
    );
    this.relatedBlogNoResultEl = this.containerEl.querySelector(
      ".header__mega-menu-search__related-blog-no-result"
    );
    this.itemTemplateEl = document.getElementById("product-search-result-item");
    this.productPrototypeEl =
      this.resultListEl?.querySelector(".product")?.cloneNode(true) || null;
    this.blogItemTemplateEl = document.getElementById(
      "blog-search-result-item"
    );
    this.blogPrototypeEl =
      this.relatedBlogListEl?.querySelector(".blog-item")?.cloneNode(true) ||
      null;
    this.viewMoreProductBtn = document.querySelector(
      ".header__mega-menu-search__result-contact"
    );
    this.viewMoreBlogBtn = document.querySelector(
      ".header__mega-menu-search__related-blog__contact-link"
    );
  ...
  bindProductVideoHover(productNode) {
    if (!productNode) return;
    const videoEl = productNode.querySelector(".product__video");
    if (!videoEl) return;
    if (productNode.dataset.videoHoverBound === "1") return;
    productNode.dataset.videoHoverBound = "1";

    const canPlay = () => {
      const src = videoEl.getAttribute("src");
      return window.innerWidth >= 1024 && !!(src && src.trim());
    };

    productNode.addEventListener("mouseenter", () => {
      if (!canPlay()) return;
      videoEl.play().catch(() => {});
    });

    productNode.addEventListener("mouseleave", () => {
      if (!canPlay()) return;
      videoEl.pause();
  ...
  createProductItemNode() {
    ...
    if (this.itemTemplateEl && "content" in this.itemTemplateEl) {
      return (
        this.itemTemplateEl.content.firstElementChild?.cloneNode(true) || null
      );
    }
    return null;
  }
  createBlogItemNode() {
    ...
    if (this.blogItemTemplateEl && "content" in this.blogItemTemplateEl) {
      return (
        this.blogItemTemplateEl.content.firstElementChild?.cloneNode(true) ||
        null
      );
    }
    return null;
  ...
  updateSearchResultsLayout(productCount, blogCount) {
    ...
    if (this.noResultEl)
      this.noResultEl.style.display = showNoResult ? "" : "none";
  ...
  renderSkeletonLoading(itemCount = HeaderMegaMenuSearch.PRODUCTS_LIMIT) {
    ...
    const count = Math.max(
      1,
      Number(itemCount) || HeaderMegaMenuSearch.PRODUCTS_LIMIT
    );
    ...
    for (let i = 0; i < count; i++) {
      ...
      const skeletonLinkEl =
        skeletonItemNode.querySelector(".product__link") || skeletonItemNode;
      if (skeletonLinkEl) {
        skeletonLinkEl.removeAttribute("href");
      }

      const skeletonVideoEl = skeletonItemNode.querySelector(".product__video");
      if (skeletonVideoEl) {
        skeletonVideoEl.pause();
        skeletonVideoEl.removeAttribute("src");
        skeletonVideoEl.load();
        skeletonVideoEl.style.display = "none";
      }

      const skeletonImageEl = skeletonItemNode.querySelector(".product__img");
      if (skeletonImageEl) {
        skeletonImageEl.setAttribute(
          "src",
          HeaderMegaMenuSearch.TRANSPARENT_PIXEL_DATA_URL
        );
        skeletonImageEl.setAttribute("alt", "");
        // Prototype clone từ wp_get_attachment_image có thể giữ srcset/sizes cũ
        skeletonImageEl.removeAttribute("srcset");
        skeletonImageEl.removeAttribute("sizes");
      }

      const skeletonTitleEl = skeletonItemNode.querySelector(".product__title");
      if (skeletonTitleEl) skeletonTitleEl.textContent = "";

      const skeletonRentPriceEl = skeletonItemNode.querySelector(
        ".product__rent-price"
      );
      if (skeletonRentPriceEl) skeletonRentPriceEl.textContent = "";

      const skeletonSalePriceEl =
        skeletonItemNode.querySelector(".product__price");
  ...
  renderBlogSkeletonLoading(itemCount = HeaderMegaMenuSearch.BLOGS_LIMIT) {
    ...
    const count = Math.max(
      1,
      Number(itemCount) || HeaderMegaMenuSearch.BLOGS_LIMIT
    );
  ...
  renderProducts(products) {
    ...
    productList.forEach((productData) => {
      ...
      // Prototype có thể là <article class="product"><a class="product__link"></a></article>
      // hoặc trực tiếp <a class="product">...</a>. Ưu tiên link con nếu có.
      const productLinkEl =
        productItemNode.querySelector(".product__link") || productItemNode;
      if (productLinkEl) {
        productLinkEl.setAttribute("href", productUrl);
      }
      productItemNode.classList.remove("product--skeleton");

      const productVideoEl = productItemNode.querySelector(".product__video");
      if (productVideoEl) {
        if (productVideoUrl) {
          productVideoEl.setAttribute("src", productVideoUrl);
          productVideoEl.style.display = "";
          productVideoEl.load();
        } else {
          productVideoEl.pause();
      ...
      const productImageEl = productItemNode.querySelector(".product__img");
      if (productImageEl) {
        ...
        // Xóa responsive attributes cũ để tránh mọi item dính cùng 1 ảnh từ prototype
        productImageEl.removeAttribute("srcset");
        productImageEl.removeAttribute("sizes");
        productImageEl.setAttribute("alt", productTitle);
      }

      const productTitleEl = productItemNode.querySelector(".product__title");
      if (productTitleEl) productTitleEl.textContent = productTitle;

      const productRentPriceValueEl = productItemNode.querySelector(
        ".product__rent-price-value"
      );
      if (productRentPriceValueEl) {
        productRentPriceValueEl.textContent = `${productRentPriceText}đ`;
      } else {
        const productRentPriceEl = productItemNode.querySelector(
          ".product__rent-price"
        );
        if (productRentPriceEl)
          productRentPriceEl.textContent = `${productRentPriceText}đ`;
      }

      const productSalePriceValueEl = productItemNode.querySelector(
        ".product__price-value"
      );
      if (productSalePriceValueEl) {
        productSalePriceValueEl.textContent = `${productSalePriceText}đ`;
      } else {
        const productSalePriceEl =
          productItemNode.querySelector(".product__price");
        if (productSalePriceEl)
          productSalePriceEl.textContent = `(Giá bán: ${productSalePriceText}đ)`;
      }

      const productSalePriceMobileEl =
        productItemNode.querySelector(".product__price-mb");
      if (productSalePriceMobileEl) {
        productSalePriceMobileEl.textContent = `( Giá bán: ${productSalePriceText}đ )`;
      }

      this.bindProductVideoHover(productItemNode);
      resultFragment.appendChild(productItemNode);
    });
  ...
  async fetchSearchResults(keyword) {
    ...
    const productsEndpointUrl = new URL(
      HeaderMegaMenuSearch.PRODUCTS_ENDPOINT,
      window.location.origin
    );
    productsEndpointUrl.searchParams.set("search", searchKeyword || "");
    productsEndpointUrl.searchParams.set(
      "limit",
      String(HeaderMegaMenuSearch.PRODUCTS_LIMIT)
    );
    productsEndpointUrl.searchParams.set(
      "page",
      String(HeaderMegaMenuSearch.PRODUCTS_PAGE)
    );

    const blogsEndpointUrl = new URL(
      HeaderMegaMenuSearch.BLOGS_ENDPOINT,
      window.location.origin
    );
    blogsEndpointUrl.searchParams.set("search", searchKeyword || "");
    blogsEndpointUrl.searchParams.set(
      "limit",
      String(HeaderMegaMenuSearch.BLOGS_LIMIT)
    );
    blogsEndpointUrl.searchParams.set(
      "page",
      String(HeaderMegaMenuSearch.BLOGS_PAGE)
    );
  ...
  handleClickSearchBtn() {
    ...
    this.btnSearchEl.addEventListener("click", () => {
      ...
      // Fetch lần đầu khi mở menu để sync thứ tự với API; các lần mở sau
      // giữ nguyên kết quả cũ, chỉ gõ vào input mới fetch lại.
  ...
  handleViewMore() {
    this.viewMoreProductBtn.addEventListener("click", () => {
      const searchValue = this.inputEl?.value?.trim() || "";
    ...
    this.viewMoreBlogBtn.addEventListener("click", () => {
      const searchValue = this.inputEl?.value?.trim();
      const url = new URL(window.location.href);
      url.searchParams.set("post_type", "blog");
      url.searchParams.set("s", searchValue);
...
const headerDesktopInit = () => {
  console.log("Header desktop init");
  createAutoHideOnScroll({
    selector: ".header",
    hiddenClassName: "header--auto-hidden",
    hideOffsetPx: 500,

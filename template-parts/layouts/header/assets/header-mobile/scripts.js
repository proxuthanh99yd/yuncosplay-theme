class HeaderMobile {
    ...
    initCategoryProductSwiper() {
        const parentSwiper = new Swiper(".header-product__category-mul-level__parent-swiper", {
            slidesPerView: 2,
            spaceBetween: remToPixels(0.5),
            freeMode: true,
            navigation: {
                nextEl: ".header-product__category-mul-level__parent-swiper-button-nav--next",
                prevEl: ".header-product__category-mul-level__parent-swiper-button-nav--prev",
            },
            watchSlidesProgress: true,
        });
        const childSwiper = new Swiper(".header-product__category-mul-level__child-swiper", {
            autoHeight: true,
            slidesPerView: 1,
    ...
    updateServiceItemHeights() {
        ...
        items.forEach((item) => {
            const content = item.querySelector(".header-service__service-item__content");
            if (!content) return;
    ...
    handleClickServiceBtnTriggerToggle() {
        ...
        this.headerServiceBtnTriggerToggleEls.forEach((btn) => {
            btn.addEventListener("click", () => {
                ...
                if (!itemEl) return;
                const isOpening = !itemEl.classList.contains("header-service__service-item--active");
                // Accordion: đóng tất cả item khác
                container.querySelectorAll(".header-service__service-item").forEach((el) => {
